...
impl PacketParser {
    ...
    pub fn parse_byte_range(&mut self, start_position: usize, length: usize) -> Result<String, Error> {
        ...
        for _ in 1..length {
            let parsed_byte = self.parse_byte().expect("Cannot parse byte.");
    ...
    pub fn parse_qname(&mut self) -> String {
        ...
        loop {
            ...
            // let label_length = self.get_byte(self.position).expect("get_byte error");
            // self.position += 1;
            let label_length = self.parse_byte().expect("parse_byte error");
            ...
            // else {
                ...
                let parsed_name = self.parse_byte_range(self.position, (label_length + 1) as usize).expect("parse_byte_range error");
